...
public actor Engine {
  ...
  /// Synchronously releases the native engine and waits for its worker pool to finish.
  ///
  /// Callers that intend to initialize a replacement GPU engine must await this method first.
  /// Relying on ARC alone can overlap native teardown with the replacement initialization.
  public func close() {
