...
heading. With that stricter evidence, the 65-case evaluation is **65/65
(100%)**, including 10/10 church-history cases, 6/6 sacraments cases, and 4/4
...
the current expanded source-routing set is **65/65 (100%) under its stricter direct-evidence
...
history, scripture, and out-of-scope screening at 100%. This measures the suite's 65 questions,
not universal factual reliability; add genuinely new phrasing and subjects before treating it as a
broader benchmark. Run the eval before and after any change to chunking, weighting, thresholds,
or corpus contents.
...
The former misses on lying, forgiveness, the Didache, and Bible reliability were all present in the
bundled corpus but missed by topic-only ranking. `SubjectSection` now maps their question terms to
literal headings or phrases in the Summa or Scripture. It never carries a conclusion, answer, or
negative example: the retrieved source text remains the only evidence shown to generation.
